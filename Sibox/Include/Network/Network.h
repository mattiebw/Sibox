...
{
	LocalOnly,
	// Only exists on the client. For example, particle systems. May have been created by the server, but isn't synced at all.
	LocalReplicated, // Exists on the server, and we are the server. We may not own it, but we do have authority.
	RemoteOwned,
	// Exists on the server, but is owned by this client, so we have special powers over it (such as destroying it).
	RemoteProxy,
	// Exists on the server, and we have no power to change it, but we receive updates on it from the server.
};
