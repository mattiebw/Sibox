...
[Serializable]
internal struct ManualFileCopy
{
    public string RelativePath { get; set; } = "";
    public string OutputRelativePath { get; set; } = "";

    public ManualFileCopy(string relativePath, string outputRelativePath = "")
    {
...
internal struct ExtraContentDirectory
{
    /// <summary>
    /// Source directory, relative to given content directory.
    /// </summary>
    public string RelativeSourcePath { get; set; } = "";
    /// <summary>
    /// Output directory, relative to given output directory.
    /// </summary>
    public string RelativeOutputPath { get; set; } = "";
    
    public ExtraContentDirectory(string relativeSourcePath, string relativeOutputPath = "")
    {
...
internal struct PreprocessorSettings
{
    ...
    /// </summary>
    public Dictionary<string, List<ManualFileCopy>> ManualCopies { get; set; } = [];
    ...
    /// <summary>
    /// Extra content directories, relative to the given content directory.
    /// </summary>
    public List<ExtraContentDirectory> ExtraContentDirectories { get; set; } = [];
...
internal static class Program
{
    ...
    public static string ContentDir = "", OriginalContentDir = "", OutputDir = "", OriginalOutputDir = "";
    ...
    private static void Main(string[] args)
    {
        ...
        OriginalContentDir = Path.GetFullPath(args[0]);
        ContentDir = OriginalContentDir;
        OriginalOutputDir = Path.GetFullPath(args[1]);
        ...
        DoManualCopies();
        DoAssetProcessing();
        foreach (var dir in PreprocessorSettings.ExtraContentDirectories)
        {
            ContentDir = Path.Join(OriginalContentDir, dir.RelativeSourcePath);
            OutputDir = Path.Join(OriginalOutputDir, dir.RelativeOutputPath);
            DoAssetProcessing();
        }
    ...
    private static void LoadProcessors()
    {
        ...
        Assembly.GetExecutingAssembly().GetTypes()
            ...
            .ForEach(t =>
            {
                if (PreprocessorSettings.DisabledProcessors.Contains(t.Name))
                {
                    Log.Trace($"Ignoring processor \"{t.Name}\" as it is disabled in the settings.");
                    return;
                }
    ...
    private static void DoManualCopies()
    {
        if (PreprocessorSettings.ManualCopies.TryGetValue("*", out var generalCopies))
            DoManualCopies(generalCopies);
        
        if (PreprocessorSettings.ManualCopies.TryGetValue(Platform, out var copies))
            DoManualCopies(copies);
    }

    private static void DoManualCopies(List<ManualFileCopy> copies)
    {
